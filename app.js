...
function navBuild() {
    function setActiveClass(e) {
        const currentSection = e.target.innerHTML;
        const selectedSections = Array.from(document.getElementsByTagName("section"));
        for (const current in selectedSections) {
    ...
    // section loop
    for (const current in selectedSections) {
        const sectionID = selectedSections[current].id;
        const sectionDataNav = selectedSections[current].dataset.nav;
